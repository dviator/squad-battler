...
import { BEAR, EAGLE, TIGER, WOLF } from "../data/species";
import { clearScreen, displayHeader } from "./display";
import { combatPhase, shopPhase } from "./phases";

const ALL_SPECIES = [BEAR, EAGLE, TIGER, WOLF, GOOB, HEAVY_GOOB, MEGA_GOOB];
...
export async function playGame(): Promise<void> {
  ...
  // Create campaign
  let campaign = createGoobCampaign(GOOB, HEAVY_GOOB, MEGA_GOOB, [BEAR, EAGLE, TIGER, WOLF]);
  ...
  while (playing && !runComplete) {
    ...
    if (!victory) {
      ...
      if (retry) {
        ...
        campaign = createGoobCampaign(GOOB, HEAVY_GOOB, MEGA_GOOB, [BEAR, EAGLE, TIGER, WOLF]);
      } else {
        playing = false;
      }
      continue;
